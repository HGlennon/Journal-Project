...
import { addTask } from '@/db/actions/add-task';
import { completeTask } from '@/db/actions/complete-task';
  ...
  async function handleAddTask(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      await startTransition(async () => {
        ...
        form.reset();
        setShowForm(true); // Ensure form stays visible after adding
      });
    } catch (err) {
...
function handleComplete(taskId: number) {
  ...
  return (
    <div>
      ...
      {/* Empty State - Only shown when no tasks exist */}
      ...
      {(tasks.length > 0 || showForm) && (
        <div className="mt-10 space-y-4 max-w-md mx-auto ml-74">
          ...
          {/* Show form either when tasks exist or when explicitly shown */}
          {(tasks.length > 0 || showForm) && (
            <form 
              ref={formRef}
